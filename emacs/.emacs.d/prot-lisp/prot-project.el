...
;; FIXME 2026-09-16: This breaks the `revert-buffer-function' as it
;; creates a new log buffer each time instead of reverting the
;; existing one.
(define-advice vc-print-root-log (:after (&rest _) prot)
  "Rename the buffer of `vc-print-root-log' to mention the project."
  (when-let* ((root (vc-root-dir))
              (_ (consp project--list))
              (_ (member root (mapcar #'car project--list))))
