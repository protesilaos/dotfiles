...
completion categories where we may want a deviation from the rule. For
those, the defaults are declared in ~completion-category-defaults~,
which I already set to nil in the previous block. We can opt for
per-category styles by configuring the user option ~completion-category-overrides~.
