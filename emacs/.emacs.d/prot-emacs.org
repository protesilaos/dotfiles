...
*** The =init.el= user option for in-buffer completion
:PROPERTIES:
:CUSTOM_ID: h:901b1d69-8ae1-4880-88a4-508b03887106
:END:

The user option ~prot-emacs-completion-in-buffer~ controls which
interface will be used to perform in-buffer completion. The details
are covered in the affected sections of my configuration:

- [[#h:804b858f-7913-47ef-aaf4-8eef5b59ecb4][The =prot-emacs-completion.el= settings for in-buffer completion popup (~corfu~)]]
- [[#h:09d0dd65-90a5-41eb-a2b9-53d00eca77df][The =prot-emacs-completion.el= back-ends for in-buffer completion with ~corfu~ (~cape~)]]
- [[#h:6d3e2d44-9b1e-4092-8287-5062e4f29da4][The =prot-emacs-completion.el= about the in-buffer completion popup (~company~)]]
...
Watch my videos:
- [[https://protesilaos.com/codelog/2023-12-12-emacs-multilingual-editing/][multilingual editing (Greek, French, Chinese, etc.)]] (2023-12-12);
- [[https://protesilaos.com/codelog/2026-07-02-emacs-french-input-method-jinx-spell/][write with input method (e.g. French) and Jinx for spelling]] (2026-07-02).

In my =prot-minibuffer.el= I define extensions to sort input methods
and group them by those I rely on versus the rest:
...
The ~package-review-policy~, which is available starting with Emacs
version 31, is a useful security enhancement. Whenever we upgrade a
package, we get the chance to produce a diff that shows the changes we
are about to pull in. The idea is that there are enough users who (i)
have the technical skills to assess the code and (ii) have the time
and willingness to do such an inspection. If that happens, then the
Emacs community as a whole becomes more robust to supply-chain
attacks, which are really common these days.
...
[ Watch: [[https://protesilaos.com/codelog/2024-01-29-emacs-prefix-map/][define prefix/leader key (nested keymaps)]] (2024-01-29). ]
...
*** COMMENT The =prot-emacs-theme.el= section for ~rainbow-mode~
:PROPERTIES:
...
*** The =prot-emacs-theme.el= section the GNOME accent theme switcher (~gnome-accent-theme-switcher~)
:PROPERTIES:
...
*** The =prot-emacs-theme.el= section for ~modus-themes-exporter~
:PROPERTIES:
:CUSTOM_ID: h:ac631ebf-1130-41ee-8c4e-441be3c01cbb
:END:

This is another package of mine. It empowers the user to export a
Modus or Modus-derived theme to another application. The idea is to
get one's favourite terminal emulator (or other such tool) to use the
colours that are shown in Emacs.

I developed this package during a live stream:
...
*** The =prot-emacs-theme.el= section for ~fontaine~
:PROPERTIES:
...
*** The =prot-emacs-theme.el= section for ~show-font~
:PROPERTIES:
...
*** The =prot-emacs-theme.el= section for ~variable-pitch-mode~ and font resizing
:PROPERTIES:
...
For the global keybinding overrides, watch my video on the matter
from 2026-07-08: <https://protesilaos.com/codelog/2026-07-08-emacs-global-keybinding-overrides/>.
...
explicitly overwritten by the user, whereas the ~kill-ring~
accumulates lots of text that we do not necessarily add there on
purpose.

I used to have extensions for registers, but have since reconsidered
my approach to do more with bookmarks. The reason is that bookmarks
are designed for persistent data, whereas registers are meant to be
ephemeral.
...
- partial-completion :: This is used for file navigation. Instead of
  ...
  It is a joy to navigate the file system in this way. Though it also
  expands words are substrings.
...
[ Watch: [[https://protesilaos.com/codelog/2026-08-09-emacs-expand-abbrev-mode-definitions-conditionally/][conditionally expand abbrev-mode definitions]] (2026-08-09). ]
...
[ Also watch my video for everything related to in-buffer completion
  from 2026-07-19: <https://protesilaos.com/codelog/2026-07-19-emacs-completion-at-point-functions/>. ]
...
Corfu is the front-end for completions. It does not implement any
function to compute the candidates for a given input. That is the
responsibility of completion backends, which are functions listed in
the value of the ~completion-at-point-functions~. Daniel Mendler has
another package, called ~cape~, which implements back-end functions
whose results can be displayed by Corfu, among others ([[#h:09d0dd65-90a5-41eb-a2b9-53d00eca77df][The =prot-emacs-completion.el= back-ends for in-buffer completion with ~corfu~ (~cape~)]]).

Note that Corfu uses child frames for its popups, which were not
supported in terminal Emacs in versions prior to 31.

The ~company~ package is an alternative to the combination of ~corfu~
and ~cape~ ([[#h:6d3e2d44-9b1e-4092-8287-5062e4f29da4][The =prot-emacs-completion.el= about the in-buffer completion popup (~company~)]]).
Because of its all-in-one approach, Company may be easier for users to
reason about and configure accordingly.
...
*** The =prot-emacs-completion.el= back-ends for in-buffer completion with or without ~corfu~ (~cape~)
:PROPERTIES:
:CUSTOM_ID: h:09d0dd65-90a5-41eb-a2b9-53d00eca77df
:END:

[ Also watch my video for everything related to in-buffer completion
  from 2026-07-19: <https://protesilaos.com/codelog/2026-07-19-emacs-completion-at-point-functions/>. ]

The ~cape~ package by Daniel Mendler defines functions that are useful
for in-buffer completion. Each function has a specific scope, such as
to match the input against file names, dictionary entries, unicode
characters, and more.

All the functions are added to the ~completion-at-point-functions~
(each function is also known as a CAPF). When Emacs performs
completion in the buffer, it calls those function one by one, from
left to right, until one returns results. Emacs stops there to display
those results. This behaviour can be confusing because we are not
seeing everything we would expect to get.

As such, Cape also provides a mechanism for a "super CAPF": a
collection of results from all the individual CAPFs. This leverages a
standard Emacs facility to make individual CAPFs inclusive. The upside
is that we get more intuitive results. The downside is that we may be
slowing down Emacs a little bit if we need to run lots of functions
for something that is ultimately not needed.

What further contributes to the potential confusion is the fact that
Emacs variables/hooks can have a buffer-local value. So what is
specific to a given buffer will take precedence over the corresponding
...
have not found a compelling reason to do this.
...
*** The =prot-emacs-completion.el= about the in-buffer completion popup (~company~)
:PROPERTIES:
:CUSTOM_ID: h:6d3e2d44-9b1e-4092-8287-5062e4f29da4
:END:

[ Also watch my video for everything related to in-buffer completion
  from 2026-07-19: <https://protesilaos.com/codelog/2026-07-19-emacs-completion-at-point-functions/>. ]

The ~company~ package by Dmitry Gutov provides a complete solution for
in-buffer completion. It consists of a front-end, which is the popup
interface, and completion back-end functions. In this regard, Company
corresponds to the combination of ~corfu~ and ~cape~:

- [[#h:804b858f-7913-47ef-aaf4-8eef5b59ecb4][The =prot-emacs-completion.el= settings for in-buffer completion popup (~corfu~)]]
- [[#h:09d0dd65-90a5-41eb-a2b9-53d00eca77df][The =prot-emacs-completion.el= back-ends for in-buffer completion with ~corfu~ (~cape~)]]

To group completion backends like we do with the Cape super CAPF we
just need to put them in a list, as I do in the ~company-backends~.
Because Company provides a ~company-capf~ function, I do not need to
consider the original value of the ~completion-at-point-functions~:
Company will just do the right thing. Otherwise, it is important to
consider what is in the ~completion-at-point-functions~ to retain
something potentially useful there (e.g. a function that is added by
some Language Server Protocol).
...
[ Also watch my video for everything related to in-buffer completion
  from 2026-07-19: <https://protesilaos.com/codelog/2026-07-19-emacs-completion-at-point-functions/>.
  I did another video on 2026-08-29 about ~completion-preview-mode~ in
  particular: https://protesilaos.com/codelog/2026-08-29-emacs-completion-preview-mode/ ]
...
These are some minor tweaks to the keymaps for isearch and occur. I
...
*** The =prot-emacs-dired.el= settings for copy, rename, delete operations
:PROPERTIES:
...
do it only when I need to draw attention to something I am
demonstrating ([[#h:b28077e7-c219-4806-91b6-b7cc0fe468ee][The =prot-emacs-window.el= section for ~prot-streaming-mode~ (=prot-streaming.el=)]]).
...
enable the mode only when I need to compare buffers or when I need to
share my screen during a video ([[#h:b28077e7-c219-4806-91b6-b7cc0fe468ee][The =prot-emacs-window.el= section for ~prot-streaming-mode~ (=prot-streaming.el=)]]).
...
*** The =prot-emacs-window.el= section for ~prot-streaming-mode~ (=prot-streaming.el=)
:PROPERTIES:
:CUSTOM_ID: h:b28077e7-c219-4806-91b6-b7cc0fe468ee
:END:

The =prot-streaming.el= library implements features that are useful
when I do a video and have to share my screen.

The ~prot-streaming-mode~ treats ~display-line-numbers-mode~ and
~hl-line-mode~ as one consistent feature, making it easy to have both
enable and disabled with a single key press.

- [[#h:f45ea51c-8129-41bd-8d6e-4b66e7f35a49][The =prot-emacs-window.el= section about line numbers (~display-line-numbers-mode~)]]
- [[#h:ec156866-3d25-42e7-8247-b190103b6f17][The =prot-emacs-window.el= section about line highlighting (~hl-line-mode~)]]
...
*** The =prot-emacs-window.el= section with relevant key bindings
:PROPERTIES:
:CUSTOM_ID: h:49ad5448-2ccd-4f47-82fd-17819ffdbdf3
:END:

Here I simply define some key bindings pertaining to window
operations. The commands I use the most in this list are
~delete-other-windows-vertically~, ~previous-buffer~, and
~next-buffer~. I rarely resize windows, though I am happy to use the
mouse for such a case.
...
*** The =prot-emacs-window.el= section for ~buffer-to-pdf~
:PROPERTIES:
:CUSTOM_ID: h:4bdb59e9-1c3c-4e60-9c11-0a6372b838db
:END:

This is yet another package of mine. It allows the user to export
their current buffer to a PDF. In practice, this is like a screen
capture of whatever is in the current buffer, paginated in a sensible
way. For example, an Org file will have one heading per page.

The font size and current theme are preserved. This way, users have a
"what you see is what you get" result. For presentations, user may
thus prefer to create a file based on the font size they like, check
that each screenfull shows the heading with its contents, and then
call the command ~buffer-to-pdf~. For printer-friendly output the
~buffer-to-pdf-black-on-white~ can be used instead.
...
:PROPERTIES:
:CUSTOM_ID: h:baf38b4b-1f0c-491f-b669-986df6af37ca
:END:

The built-in ~speedbar~ package defines infrastructure to persistently
display information related to the current buffer.

Without any configuration, this is the file structure related to the
current buffer plus the ~imenu~ outline.

Users can also list the buffers, though I have no use for that feature
given that the same can be achieved, if needed, with ~list-buffers~ or
~ibuffer~. To me, the ~speedbar~ interface is useful for its capacity
to show the relevant subtree of the filesystem.

Call the command ~speedbar~ to get started.

By default, ~speedbar~ appears in a separate frame, which I consider
impractical. Starting with Emacs version 31, however, there is a user
option to move it to a side window (~speedbar-prefer-window~). I
expect this alone to make ~speedbar~ useful for more users.

The manual documents how we can extend ~speedbar~ to show arbitrary
data, though I have no such need at the moment.

The following tweaks make ~speedbar~ work in a manner that I consider
intuitive. I remove the bitmap images it shows on graphical displays
because I find them distracting. For the Imenu index, I choose to
flatten it because I do not like the default grouping behaviour.
...
*** The =prot-emacs-git.el= section about ~diff-hl~
:PROPERTIES:
:CUSTOM_ID: h:8e28de42-740d-47ea-b88f-f9fd8d9b6f4f
:END:

The ~diff-hl~ package by Dmitry Gutov applies a highlight to the side
of the window on lines that have changes, as tracked by the underlying
version control software (it is always Git in my case, but it works
with all VCS that Emacs supports ([[#h:50add1d8-f0f4-49be-9e57-ab280a4aa300][The =prot-emacs-git.el= section about =vc.el= and related]])).

I have no use for it while writing prose, so I only enable it in
programming buffers. The highlights are especially useful in large
files where you need to jump around to understand and to modify the
...
*** The =prot-emacs-git.el= section about =vc.el= and related (including =prot-vc.el=)
:PROPERTIES:
...
My =prot-vc.el= adds minor refinements and a few commands on top of
this corpus of functionality ([[#h:4405e6d5-6bd9-4fbe-8d42-c788b3dd2ee7][The =prot-vc.el= library]]).
...
*** The =prot-emacs-org.el= section about Institution Calendar (~institution-calendar~)
:PROPERTIES:
:CUSTOM_ID: h:a9b19aca-6705-42c8-a24a-73253ae30226
:END:

The ~institution-calendar~ is another one of my many packages. It
augments the regular =M-x calendar= with term+week numbers for one's
institution of choice. This is typically useful for schools and
universities that organise their yearly work by terms, each with its
own number of weeks. (e.g. the University of Oxford counts the week of
2026-02-11 as week 2 of the Hilary term). The manual shows how you can
define your own institution. The universities of Oxford and Cambridge
...
makes them look curly/prettier.

I tried to use all of them in earnest and have concluded that I only
like the behaviour of ~electric-indent-mode~ in the context of
programming. The ~electric-indent-mode~ is the global name, so in a
programming mode I am opting for ~electric-indent-local-mode~. Note
that this is only for programming, because I never want to do the same
with prose.

The other modes are not suitable to my workflow. The ~electric-quote-mode~
will change the characters I am inputting, which I do not like. As for
the ~electric-pair-mode~, I did not like two aspects of its behaviour:

- For the paired characters, the highlighted region is not deleted but
  is instead wrapped in the given characters. This breaks my flow
  because now I need to think which are those cases. Whereas I like
  ~delete-selection-mode~ to always have the same effect ([[#h:d551b90d-d730-4eb5-976a-24b010fd4db3][The =prot-emacs-essentials.el= section for ~delete-selection-mode~]]).
- Balancing parentheses in Lisp as you insert them is very nice.
  Though it becomes trickier when you need to modify existing code.
  Then you need to rely on marking the code and inserting the
  parentheses around it, as noted above. Whereas I find it easier to
  just write on a previous line and then move to where I want the
  expression to end and insert a closing delimiter there.
...
Also see Jinx, which is most probably better than Flyspell: [[#h:7f027040-7cd1-4b38-99b5-cb0e17f8866d][The =prot-emacs-langs.el= sample configuration for the Jinx spell checker]].
...
*** The =prot-emacs-langs.el= sample configuration for the Jinx spell checker
:PROPERTIES:
:CUSTOM_ID: h:7f027040-7cd1-4b38-99b5-cb0e17f8866d
:END:

I use the generic Flyspell that is built into Emacs to do spell
checking on demand after I am done writing. Though in my experience
that is not the workflow most people want. They prefer to have spell
checking on the fly, which is also capable of working with multiple
languages at once. This is why I recommend the ~jinx~ package by
Daniel Mendler, which I demonstrated in a video I did about writing
using an input method (e.g. French) and using Jinx for spell-checking:
<https://protesilaos.com/codelog/2026-07-02-emacs-french-input-method-jinx-spell/>.
...
keep everything in sync for no apparent benefit. One MUA is enough
([[#h:b26c6297-ebe8-4f99-b069-df53cfd90520][The =prot-emacs-email.el= submodule for ~notmuch~ (=prot-emacs-notmuch.el=)]]).
...
*** The deprecated =prot-emacs-email.el= submodule for ~mu4e~ (=prot-emacs-mu4e.el=)
:PROPERTIES:
...
*** The deprecated =prot-emacs-email.el= submodule for Gnus (=prot-emacs-gnus.el=)
:PROPERTIES:
...
** COMMENT The =prot-register.el= library
:PROPERTIES:
...
** The =prot-streaming.el= library
:PROPERTIES:
:CUSTOM_ID: h:bccb4b34-325e-43ba-b08f-b7da5342b522
:END:

#+begin_src emacs-lisp :tangle "prot-lisp/prot-streaming.el" :mkdirp yes
...
** The =prot-vc.el= library
:PROPERTIES:
:CUSTOM_ID: h:4405e6d5-6bd9-4fbe-8d42-c788b3dd2ee7
:END:

#+begin_src emacs-lisp :tangle "prot-lisp/prot-vc.el" :mkdirp yes
