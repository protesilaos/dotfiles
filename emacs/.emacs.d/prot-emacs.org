...
~completion-styles~. In the next section, we will set up a more
fine-grained set of completion styles on a per-category basis
([[#h:d178548f-affe-41cc-a92a-d8a255dbe86c][The =prot-emacs-completion.el= settings for completion category overrides]]).
...
:PROPERTIES:
:CUSTOM_ID: h:d178548f-affe-41cc-a92a-d8a255dbe86c
:END:

The ~completion-styles~ is the fallback option in case there is no
provision for the given completion category ([[#h:14b09958-279e-4069-81e3-5a16c9b69892][The =prot-emacs-completion.el= settings for completion styles]]).
The completion category is a piece of metadata that is associated with
the completion table we are matching against while using the
minibuffer. For example, the ~find-file~ command has the =file=
category, while the ~switch-to-buffer~ command uses the =buffer=
category. The defaults for those are specified in the variable
...
*** The =prot-emacs-completion.el= for in-buffer completion popup and preview (~corfu~)
:PROPERTIES:
...
What the handles the popup window, though we can enhance it by using a
facility built into Emacs 30 that previews the first candidate. This
can be helpful, as we may have already typed what matches the target,
so we just follow it up with =TAB= to complete the operation.
