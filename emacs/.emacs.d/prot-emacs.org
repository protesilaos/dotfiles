...
When I am working on my laptop, I want all my frames to be maximised
by default ([[#h:0403b581-142b-4613-bc81-51c5dccd831d][The =early-init.el= code to set frame parameters]]).
I also arrange for the battery indicator to be displayed on the mode
line. For a while, I was using the ~prot-laptop-p~ to conditionally
load my ~beframe~ package only on the desktop, but have decided to
switch back to using it everywhere---I like its workflow more ([[#h:77e4f174-0c86-460d-8a54-47545f922ae9][The =prot-emacs-window.el= section about ~beframe~]]).
