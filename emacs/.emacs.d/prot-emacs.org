...
either from one of the package archives or directly from source. Below
are some examples of how it can be used.

Install the ~modus-themes~ package as a tarball from the relevant
package archive ([[#h:424340cc-f3d7-4083-93c9-d852d40dfd40][The =init.el= settings for packages (=package.el=)]]).

#+begin_example emacs-lisp
(prot-emacs-install modus-themes)
#+end_example

Install the package from the given URL, which is a Git repository. In
this case, the name of the package and the appropriate version control
software are retrieved from the URL.

#+begin_example emacs-lisp
(prot-emacs-install ("https://github.com/protesilaos/modus-themes.git"))
#+end_example

This is the same method as above, except that (i) it specifies the
name of the package, which could be different from that implied by the
URL, and (ii) demands a specific branch from that repository.

#+begin_example emacs-lisp
(prot-emacs-install (modus-themes :url "https://github.com/protesilaos/modus-themes.git" :branch "main"))
#+end_example

With source code installations, we also have the option to build a
package from a local clone of the repository.

#+begin_example emacs-lisp
(prot-emacs-install ("~/Git/Projects/modus-themes"))
#+end_example

Finally, we can give it the name of the local package, if the
directory we are specifying results in some other name that we do not
want to use.

#+begin_example emacs-lisp
(prot-emacs-install ("~/Git/Projects/modus-themes-demo-directory" "modus-themes"))
#+end_example
