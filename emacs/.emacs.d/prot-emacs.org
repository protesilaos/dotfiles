...
Emacs buffer. Without ~ready-player-mode~, we get a bunch of gibberish
...
packages. When I move the cursor over a multimedia file, the Dired
preview buffer tries to visit the file at point, which is in turn
rendered using ~ready-player~.
