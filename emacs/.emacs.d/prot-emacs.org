...
*** The =prot-emacs-essentials.el= section about ~exec-path-from-shell~
:PROPERTIES:
:CUSTOM_ID: h:f2447060-5d89-48d5-a682-5cfc76e0928e
:END:

The ~exec-path-from-shell~ package by Steve Purcell gets environment
variables from the system shell and makes them available to Emacs. I
have found that I need this while running on Wayland to properly use
SSH, but maybe I can do without it if I tweak a few things in
=~/.config/environment.d=envvars.conf=... I will check.
...
*** TODO The =prot-emacs-completion.el= section about ~embark~
:PROPERTIES:
...
any subdirectory, such as to benefit of the bulk-renaming capability,
(v) running a keyboard macro that edits file contents while using
Dired to navigate their list, (vi) open files in an external
application, and more.

Dired lets us operate on our files in a way that still feels close to
the command-line, yet has more powerful interactive features than
fully fledged, graphical file managers. The only area where it is not
as good is in previewing multimedia files, though I anyway rely on my
Denote file-naming convention to find what I am looking for (far more
reliable than a preview, plus it is easy to open the video in a media
player).
...
*** TODO The =prot-emacs-dired.el= section about various conveniences
...
*** TODO The =prot-emacs-dired.el= section about ~dired-subtree~
...
*** TODO The =prot-emacs-dired.el= section about ~wdired~ (writable Dired)
...
*** TODO The =prot-emacs-dired.el= section about ~image-dired~
...
*** TODO The =prot-emacs-dired.el= section about =trashed.el=
...
*** TODO The =prot-emacs-dired.el= section about ~dired-preview~
...
*** TODO The =prot-emacs-dired.el= section about ~mandoura~ (~mpv~ media player)
...
**** TODO The =prot-emacs-notmuch.el= section about the general user interface
:PROPERTIES:
...
** The =prot-emacs-icons.el= module
:PROPERTIES:
