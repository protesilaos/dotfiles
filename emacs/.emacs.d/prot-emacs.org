...
NOTE 2025-10-28: Here I also configure my ~prot-elfeed~ extension,
which has some custom code ([[#h:98c313d7-62c2-48c1-b6ad-28cc25f22092][The =prot-elfeed.el= library]]). Though I
need to review it. The one part that I will keep in some form, no
matter the changes I might make, is how I put all feeds in their own
file and load it separately from my Emacs configuration. This is an
Emacs Lisp file which does a ~(setq elfeed-feeds ...)~, as shown in
the example above. Doing ~(load-file FILE)~ has the effect of
evaluating the Elisp therein.
