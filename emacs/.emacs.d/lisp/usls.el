...
(defun usls--slug-no-punct (str)
  ...
  (replace-regexp-in-string "[[:punct:]]*" "" str))

(defun usls--slug-hyphenate (str)
  "Replace spaces with hyphens in STR."
  (replace-regexp-in-string "--+" "-" (replace-in-string " " "-" str)))
...
;;;###autoload
(defun usls-id-insert (file)
  "Insert at point the identity (timestamp) of FILE."
  (interactive
   (list (completing-read "Link to: "
           (usls--directory-files-not-current) nil t nil usls--link-history)))
  (let ((id (usls-extract usls-id-regexp file)))
    ...
    (save-excursion
      ...
      (insert
       (format "^^ %s\n" file)))
    (add-to-history 'usls--link-history id)))
...
(defface usls-header-data-primary
  ...
  "Face for displaying primary header information.")

(defface usls-header-data-secondary
  '((t :inherit (bold shadow)))
  "Face for displaying secondary header information.")
